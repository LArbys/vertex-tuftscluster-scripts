...
# Cut strings
#
cutstr_1 = ' npar==2 and nprotons==1 and nothers==0'
...
for the_pair in [("ALL",comb_df),("GOOD",good_df),("BAD",bad_df)]:
    ...
    den_   = the_df.query("nprotons==1 and nothers==0").index.size
    num_   = len(the_df.query("nprotons==1 and nothers==0").groupby(rse))
    ratio_ = 0
    if den_ > 0:
