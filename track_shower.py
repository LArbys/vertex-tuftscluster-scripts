...
# Truth filter
truthcut = "nprotons==1 and nothers==0"
...
for name,file_ in [(sample_name,sample_file)]:
    ...
    dfs[name] = comb_df.query(truthcut).copy()
    ...
    print "@ sample:",name
    print " total events (EventVertexTree): ", len(event_vertex_df.groupby(rse))
    print " total events (CombinedVertexTree): ",len(comb_df.groupby(rse))
    print " 1l1p events (EventVertexTree): ", event_vertex_df.query(truthcut).index.size
    print " 1l1p events (CombinedVertexTree): ", len(comb_df.query(truthcut).groupby(rse))
    print " good croi (EventVertexTree):",event_vertex_df.query("good_croi_ctr>0 and "+truthcut).index.size
    #print " good vertex:",event_vertex_df.query("good_croi_ctr>0 and min_vtx_dist<5.0 and "+truthcut).index.size
    print " good vertex (EventVertexTree):",event_vertex_df.query("good_croi_ctr>0 and min_vtx_dist<5.0 and "+truthcut).index.size
    print " good vertex (CombinedVertexTrees):",len(comb_df.query("scedr<5.0 and "+truthcut).groupby(rse))
...
print comb_df.query(ts_query).groupby(rse)
