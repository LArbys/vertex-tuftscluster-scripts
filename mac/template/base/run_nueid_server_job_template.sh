...
python ${nueid_inter_dir}/inter_ana_nue_server.py ${input_supera_file} ${input_ssnet_file} ${input_tagger_file} ${input_vertex_file} ${input_reco2d_file} ${nueid_cfg_file} ${job_id} BBB . >> $logfile 2>&1
chmod -R a+rwx ${output_dir}
...
python ${shower_dir}/reco_recluster_shower.py nueid_lcv_out_${jobid}.root ${input_reco2d_file} INVALID . 0 0 BBB ${shower_cfg_file} >> $logfile 2>&1
chmod -R a+rwx ${output_dir}
rc=$?; if [[ $rc != 0 ]]; then exit $rc; fi
...
python ${shower_dir}/run_ShowerQuality.py shower_reco_out_${jobid}.root ${input_reco2d_file} INVALID . >> $logfile 2>&1
chmod -R a+rwx ${output_dir}
rc=$?; if [[ $rc != 0 ]]; then exit $rc; fi
...
python ${match_dir}/ana_truth_match_server.py ${shower_ana_cfg_file} shower ${input_supera_file} ${input_ssnet_file} shower_reco_out_${jobid}.root . >> $logfile 2>&1
chmod -R a+rwx ${output_dir}
rc=$?; if [[ $rc != 0 ]]; then exit $rc; fi
...
python ${pid_dir}/inference_pid.py ${input_supera_file} nueid_lcv_out_${jobid}.root . ${inference_cfg_file} >> $logfile 2>&1
chmod -R a+rwx ${output_dir}
rc=$?; if [[ $rc != 0 ]]; then exit $rc; fi
...
python ${pid_dir}/inference_multiplicity.py ${input_supera_file} nueid_lcv_out_${jobid}.root . ${inference_cfg_file} >> $logfile 2>&1
chmod -R a+rwx ${output_dir}
rc=$?; if [[ $rc != 0 ]]; then exit $rc; fi
...
echo "python ${stage2_ana_dir}/make_nueid_pickle.py showerqualsingle_${jobid}.root shower_truth_match_${jobid}.root multipid_out_${jobid}.root multiplicity_out_${jobid}.root nueid_ana_${jobid}.root flash_ana_nue_${jobid}.root ." >> $logfile
python ${stage2_ana_dir}/make_nueid_pickle.py showerqualsingle_${jobid}.root shower_truth_match_${jobid}.root multipid_out_${jobid}.root multiplicity_out_${jobid}.root nueid_ana_${jobid}.root flash_ana_nue_${jobid}.root . >> $logfile 2>&1
chmod -R a+rwx ${output_dir}
rc=$?; if [[ $rc != 0 ]]; then exit $rc; fi
