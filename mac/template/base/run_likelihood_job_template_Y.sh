...
echo "run nue ll Y selection..." >> $logfile
echo "python ${nue_ll_dir}/run_nue_likelihood_Y.py nue_selected_df_${jobid}.pkl ${nue_ll_dir}/bin/revised_LLem_Y_pdfs.root ${nue_ll_dir}/bin/revised_LLpc_Y_pdfs.root ${jobid} ." >> $logfile
python ${nue_ll_dir}/run_nue_likelihood_Y.py nue_selected_df_${jobid}.pkl ${nue_ll_dir}/bin/revised_LLem_Y_pdfs.root ${nue_ll_dir}/bin/revised_LLpc_Y_pdfs.root ${jobid} . >> $logfile 2>&1
rc=$?;
chmod 777 *
echo "return code=${rc}" >> $logfile
if [[ $rc != 0 ]]; then exit $rc; fi
