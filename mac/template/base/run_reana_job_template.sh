...
echo "input files: ${input_files}" >> $logfile
...
# command
echo "RUNNING: python run_reana.py ${cfg_file} ${outfile_ana_temp} ${outfile_out_temp} -1 -1 ${input_file}" >> $logfile

# RUN
python run_reana.py ${cfg_file} ${outfile_ana_temp} ${outfile_out_temp} -1 -1 ${input_files} >> $logfile 2>&1 || exit
