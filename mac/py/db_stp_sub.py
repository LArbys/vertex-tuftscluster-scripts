...
# stage1
#
ret_v = get_stage1_flist(NAME,IS_MC);
...
targ_lc_num_v   = ["%05d" % f['jobtag'] for f in ret_v if f[VALID] != ""]
...
PREFIX = 'ana_comb_df'
targ_comb_pkl_flist_v = [f[PREFIX] for f in ret_v if f[VALID] != ""]
targ_comb_pkl_num_v   = ["%05d" % f['jobtag'] for f in ret_v if f[VALID] != ""]
...
for accid in xrange(NACC):
    ...
    for x in zip(targ_ll_reco_num_slice,targ_ll_reco_flist_slice,
        ...
        f = open(os.path.join(inputlists_dir,"reco_inputlist_%05d.txt" % int(targ_lc_num)),"w+")
        f.write(targ_lc_ss_flist)
        f.close()
        del f

        f = open(os.path.join(inputlists_dir,"vertex_inputlist_%05d.txt" % int(targ_lc_num)),"w+")
        f.write(targ_lc_flist)
        f.close()
        del f

        f = open(os.path.join(inputlists_dir,"pkl_inputlist_%05d.txt" % int(targ_lc_num)),"w+")
    ...
    data = data.replace("BBB",str(IS_MC))
