...
OUT_FOLDER = "%s_reco_%s" % (NAME,TYPE)
OUT_DIR    = os.path.join(PY_DIR,"../..",OUT_FOLDER)

SCRIPT_DIR = os.path.join(MAC_DIR,"submit_reco")
...
flist_v = [f[PREFIX] for f in ret_v if f[PREFIX] != ""]
num_v   = ["%05d" % f['jobtag'] for f in ret_v if f[PREFIX] != ""]
...
for accid in xrange(NACC):
    ...
    for num,flist in zip(num_slice,flist_slice):
        f = open(os.path.join(inputlists_dir,"inputlist_%05d.txt" % int(num)),"w+")
        f.write(flist)
        f.close()
        del f
